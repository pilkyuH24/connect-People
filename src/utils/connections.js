    ...
    for (
      ...
      if (distance < connectionRange) {
        ...
        if (hasCommonHobbies && hasCommonLocation) {
          ...
          ctx.strokeStyle = "rgba(255, 0, 0, 0.8)"; // Red for both
        } else if (hasCommonHobbies) {
          // Only hobbies overlap
          if( filters.hobbies){
            ctx.strokeStyle = "rgba(0, 0, 255, 0.8)"; // Blue for hobbies
          }else{
          ...
          if( filters.location){
            ctx.strokeStyle = "rgba(0, 255, 0, 0.8)"; // Green for location
          } else{
            ctx.strokeStyle = "rgba(0, 0, 0, 0)";
          }
          ...
          ctx.strokeStyle = "rgba(0, 0, 0, 0.8)"; // black for nationality
          }else{
            ctx.strokeStyle = "rgba(0, 0, 0, 0)";
          }
        } else {
        ...
        if (
          ...
          ctx.lineWidth = 3 - distance / connectionRange; // Thicker line for emphasis
        } else {
          ctx.lineWidth = 1.2 - distance / connectionRange; // Default line width
        }
